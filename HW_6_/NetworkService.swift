...
final class NetworkService {
    ...
    func getFriends(completion: @escaping(Result<[Friend], Error>) -> Void) {
        guard let url = URL(string:
                                "https://api.vk.com/method/friends.get?fields=photo_400,online&access_token=\(NetworkService.token)&v=5.131") else {
            return
        }
    ...
    func getProfileInfo(completion: @escaping(User?) -> Void) {
        guard let url = URL(string:
                                "https://api.vk.com/method/users.get?fields=photo_400,orig&access_token=\(NetworkService.token)&v=5.131") else {
            return
        }
        session.dataTask(with: url) { (data, _, error) in
            guard let data = data else {
